...
1. Go to the [**Security** tab](https://github.com/dhh1128/entviz-java/security) of this repository.
...
The `entviz` artifact (`io.github.dhh1128:entviz`) is pre-1.0 and ships from the
`main` branch. Security fixes are applied to `main` and released as a new
version on Maven Central; there are no long-term support branches. Please ensure
you are running the latest published version before reporting.
...
filesystem access, spawns no processes, and uses only the Java standard library
(it implements Keccak-256 itself; SHA-512 comes from the JDK). The main trust
boundary is the **untrusted input string** and the fact that the output SVG is
typically **embedded into an HTML page** by a downstream consumer.
...
- **Input is untrusted.** Callers may pass arbitrary, attacker-controlled
  ...
  - **Output injection** — the only free-text that reaches the SVG is the
    optional `note`. `Pipeline.sanitizeNote` is a MUST-reject gate: a note is
    rejected unless it is printable ASCII (U+0020–U+007E) and at most 10 code
    points. All other text written into the SVG (cell glyphs, labels) is
    XML-escaped via `escAttr` (escapes `& < > "`) for attribute contexts and
    `escText` (escapes `& < >`) for element-text contexts, so an input cannot
    break out of its markup context and inject elements/attributes.
...
- **Accepted risk — host-page CSS override.** The SVG carries all visual
  ...
  library; consumers that need to defend against it should isolate the SVG (e.g.
...
- **Out of scope.** entviz makes no cryptographic security claim: it is a
  *visualization* of entropy for human comparison, not a commitment, MAC, or
  authentication primitive. The SHA-512 fingerprint and Keccak-256 (used only
  for EIP-55 checksum validation) derive deterministic visual structure, not a
  security guarantee.
