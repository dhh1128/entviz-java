 ...
 * MOD 97-10 pair) failed to verify.
 *
 * <p>A parser may surface a bound checksum only if it VERIFIED it; a structural
 * match with a bad checksum is an error, not a value to render with a bad
 * checksum on display — matching the existing EIP-55 behavior. The
 ...
 * the input as an error vector. See docs/spec.md "Checksum verification".
 */
